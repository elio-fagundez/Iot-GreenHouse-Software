...
export function GraphicSuscribers({ data, title, fill }: GraphicSuscribersProps) {
    ...
    return (
        <div className="mt-2 h-[300px] lg:h-auto">
            <p className="text-3xl mb-3">{title}</p>
            <div className="mb-4">
                <label htmlFor="days" className="mr-2">Select days:</label>
                <select id="days" value={days} onChange={handleDaysChange} className='bg-[#3a414a]'>
                    <option value={7}>7 days</option>
                    <option value={15}>15 days</option>
                    <option value={30}>30 days</option>
                </select>
                <br />
                <label htmlFor="date" className="mr-2">Or select a date:</label>
                <input type="date" className='bg-[#3a414a]'  id="date" value={selectedDate || ''} onChange={handleDateChange} />
            </div>
                ...
                <div className="h-[200px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                            ...
                            <Line type="monotone" dataKey={title} stroke="#008d34" fillOpacity={1} fill={fill ? fill : "url(#colorUv)"} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            ) : (
